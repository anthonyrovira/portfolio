...
export const SignInOrUpForm = (props: Props) => {
  const signInOptions = useMemo(() => constructSignInOptions(props.signInOptions), [props.signInOptions]);
  ...
  const openTosUrl = config.tosLink
    ? () => {
        const tosLink = config.tosLink?.startsWith("/") ? `${__APP_BASE_PATH__}/${config.tosLink}` : config.tosLink;
